...
export function WildlifeTrips(){
 const{session}=useAuth();const nav=useNavigate();const[items,setItems]=useState<WildlifeTrip[]>([]);const[query,setQuery]=useState('');const[sort,setSort]=useState('newest');const[error,setError]=useState('');
 useEffect(()=>{if(session)wildlifeApi.trips(session.accessToken).then(setItems).catch(e=>setError(e instanceof Error?e.message:String(e)))},[session]);
...
export function WildlifeTripDetails(){
 const{id=''}=useParams();const{session}=useAuth();const nav=useNavigate();const[a,setA]=useState<Attendance>();const[record,setRecord]=useState<MonitoringRecord>();const[form,setForm]=useState({ticketNumber:'',tidNumber:'',monitoringOfficer:'',supervisor:''});const[error,setError]=useState('');const[busy,setBusy]=useState(false);
 ...
 useEffect(()=>{if(!session)return;wildlifeApi.records(session.accessToken).then(rows=>{const existing=rows.find(x=>x.tripId===id);if(existing){setRecord(existing);setForm({ticketNumber:existing.ticketNumber,tidNumber:existing.tidNumber,monitoringOfficer:existing.monitoringOfficer,supervisor:existing.supervisor})}}).catch(()=>undefined)},[session,id]);
 const request=async()=>{if(!session)return;setBusy(true);setError('');try{let r=record;if(!r)r=await wildlifeApi.create(session.accessToken,{tripId:id,...form});setRecord(await wildlifeApi.requestSignature(session.accessToken,r.id,form))}catch(e){setError(e instanceof Error?e.message:'Unable to save')}finally{setBusy(false)}};
