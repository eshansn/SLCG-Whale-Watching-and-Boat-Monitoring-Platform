...
export function WildlifeTripDetails(){
 const{id=''}=useParams();const{session}=useAuth();const nav=useNavigate();const[attendanceData,setAttendanceData]=useState<{requestedTripId:string;attendance?:Attendance;error:string}>({requestedTripId:'',error:''});const[attendanceLoadingState,setAttendanceLoading]=useState(true);const attendanceRequestVersion=useRef(0);const[storedRecord,setRecord]=useState<MonitoringRecord>();const[form,setForm]=useState({ticketNumber:'',tidNumber:'',monitoringOfficer:'',supervisor:''});const[formTripId,setFormTripId]=useState('');const[error,setError]=useState('');const[busy,setBusy]=useState(false);const[recordLoad,setRecordLoad]=useState({requestedTripId:'',error:''});const[recordsLoading,setRecordsLoading]=useState(true);const[recordsRetry,setRecordsRetry]=useState(0);const attendanceIsCurrent=attendanceData.requestedTripId===id;const a=attendanceIsCurrent?attendanceData.attendance:undefined;const attendanceError=attendanceIsCurrent?attendanceData.error:'';const attendanceLoading=attendanceLoadingState||!attendanceIsCurrent;const recordIsCurrent=recordLoad.requestedTripId===id;const record=recordIsCurrent?storedRecord:undefined;const recordError=recordIsCurrent?recordLoad.error:'';const recordLoading=recordsLoading||!recordIsCurrent;const currentForm=formTripId===id?form:{ticketNumber:'',tidNumber:'',monitoringOfficer:'',supervisor:''};
 const load=useCallback(()=>{if(!session)return;const currentRequest=++attendanceRequestVersion.current;void wildlifeApi.attendance(session.accessToken,id).then(attendance=>{if(currentRequest!==attendanceRequestVersion.current)return;setAttendanceData({requestedTripId:id,attendance,error:''})}).catch(loadError=>{if(currentRequest!==attendanceRequestVersion.current)return;const message=loadError instanceof Error?loadError.message:'Unable to load the latest attendance.';setAttendanceData(current=>current.requestedTripId===id?{...current,error:message}:{requestedTripId:id,error:message})}).finally(()=>{if(currentRequest===attendanceRequestVersion.current)setAttendanceLoading(false)})},[session,id]);useEffect(()=>{load();if(!session)return;const disconnect=watchAttendance(session.accessToken,load);return()=>{attendanceRequestVersion.current+=1;disconnect()}},[load,session]);
 ...
 const retryAttendance=()=>{setAttendanceLoading(true);load()};
 const request=async()=>{if(!session||recordLoading||recordError)return;setBusy(true);setError('');try{let r=record;if(!r){r=await wildlifeApi.create(session.accessToken,{tripId:id,...currentForm});setRecord(r)}setRecord(await wildlifeApi.requestSignature(session.accessToken,r.id,currentForm))}catch(e){setError(e instanceof Error?e.message:'Unable to save')}finally{setBusy(false)}};
 ...
 const attendanceAlert=attendanceError&&<div role="alert" className="flex items-center justify-between gap-3 rounded-lg bg-red-50 p-4 text-sm text-red-700"><span>{attendanceError}</span><button type="button" disabled={attendanceLoading} onClick={retryAttendance} className="shrink-0 font-semibold underline disabled:opacity-50">{attendanceLoading?'Retrying…':'Retry'}</button></div>;
 if(!a)return <Shell title="Trip monitoring" sub={attendanceLoading?'Loading latest attendance…':'Attendance unavailable'}>{attendanceAlert||<p className="text-sm text-slate-400">Loading latest attendance…</p>}</Shell>;
