...
export default function BoatDetailsPage() {
  ...
  const boat = records.boats.find(item => item.id === Number(boatId ?? fleetId ?? id));
  ...
  return <main className="mx-auto max-w-7xl px-6 py-10">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <div className="space-y-6">
        ...
        <Card title="Certifications"><div className="mt-4 grid gap-3 sm:grid-cols-2">{boat.documents?.map(item => <article key={item.id} className="rounded-lg bg-slate-50 p-4"><p className="font-semibold">{item.name}</p><p className="truncate text-xs text-slate-500">{item.fileName}</p><div className="mt-3 flex gap-2"><button onClick={() => void openDocument(item.id)} aria-label={`View ${item.name}`} title="View" className="rounded-full p-2 text-indigo-700"><Eye size={18}/></button><button onClick={() => void openDocument(item.id, true)} aria-label={`Download ${item.name}`} title="Download" className="rounded-full p-2 text-indigo-700"><Download size={18}/></button></div></article>)}</div></Card>
