...
export function AdminRecordsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      const usesWildlifeApprovals = session.roles.includes("Wildlife") && !session.roles.includes("Admin");
      ...
      const boats: BoatRecord[] = apiBoats.map((boat) => {
        const portalApproval = usesWildlifeApprovals ? boat.wildlifeApproval : boat.approval;
        return { id: boat.id, apiId: boat.id, imageUrl: boat.imageUrl, documents: boat.documents, name: boat.name, registrationNumber: boat.registrationNumber, registrationDate: boat.registrationDate, hullNumber: boat.hullNumber, length: `${boat.lengthMeters} m`, width: `${boat.widthMeters} m`, capacity: boat.maximumCapacity, ownerId: boat.ownerId, crewIds: crewDirectory.filter((member) => member.boatId === boat.id).map((member) => member.id), approval: portalApproval === "Rejected" ? "Declined" : portalApproval as ApprovalStatus, certifications: boat.documents.map((document) => document.name), tripIds: apiTrips.filter((trip) => trip.boatId === boat.id).map((trip) => trip.id) };
      });
