...
interface RecordsContextValue extends RecordsState {
  ...
  setBoatApproval: (id: number, status: ApprovalStatus, reason?: string) => Promise<void>; setCrewApproval: (id: number, status: ApprovalStatus, reason?: string) => Promise<void>;
  deleteBoat: (id: number) => Promise<void>; deleteCrew: (id: number) => void; deleteOwner: (id: number) => boolean;
}
...
export function AdminRecordsProvider({ children }: { children: ReactNode }) {
 ...
 useEffect(()=>{
  ...
  let active=true;
  const load=()=>void Promise.all([operationsApi.boats(session.accessToken),operationsApi.trips(session.accessToken),operationsApi.directory(session.accessToken),session.roles.includes("Admin")?operationsApi.adminCrew(session.accessToken):Promise.resolve(null)]).then(([apiBoats,apiTrips,directory,adminCrew])=>{
   if(!active)return;
 ...
 const value:RecordsContextValue={...state,updateBoat:(id,v)=>patch("boats",id,v),updateCrew:(id,v)=>patch("crew",id,v),updateOwner:(id,v)=>patch("owners",id,v),setBoatApproval:async(id,approval,declineReason)=>{const boat=state.boats.find(x=>x.id===id);if(!boat?.apiId||!session)throw new Error('The boat approval could not be updated.');await operationsApi.approveBoat(session.accessToken,boat.apiId,approval==="Approved"?"Approved":"Rejected",declineReason);patch("boats",id,{approval,declineReason:approval==="Declined"?declineReason:undefined})},setCrewApproval:async(id,approval,declineReason)=>{const member=state.crew.find(x=>x.id===id);if(!member?.apiId||!session)throw new Error('The crew approval could not be updated.');await operationsApi.approveCrew(session.accessToken,member.apiId,approval==="Approved"?"Approved":"Rejected",declineReason);patch("crew",id,{approval,declineReason:approval==="Declined"?declineReason:undefined})},deleteBoat:async id=>{const boat=state.boats.find(x=>x.id===id);if(!boat?.apiId||!session)throw new Error('The boat could not be deleted.');await operationsApi.deleteBoat(session.accessToken,boat.apiId);setState(s=>({...s,boats:s.boats.filter(b=>b.id!==id),owners:s.owners.map(o=>({...o,boatIds:o.boatIds.filter(x=>x!==id)})),crew:s.crew.map(c=>c.boatId===id?{...c,boatId:undefined}:c)}))},deleteCrew:id=>setState(s=>({...s,crew:s.crew.filter(c=>c.id!==id),boats:s.boats.map(b=>({...b,crewIds:b.crewIds.filter(x=>x!==id)}))})),deleteOwner:id=>{if(state.boats.some(b=>b.ownerId===id))return false;setState(s=>({...s,owners:s.owners.filter(o=>o.id!==id)}));return true;}};
