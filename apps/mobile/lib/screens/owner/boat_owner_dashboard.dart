...
class _BoatOwnerDashboardState extends State<BoatOwnerDashboard> {
  ...
  Widget build(BuildContext context) {
    final active = store.trips
        .where((trip) => trip.status == OwnerTripStatus.active)
        .toList();
    final upcoming = store.trips
        .where((trip) => trip.status == OwnerTripStatus.upcoming)
        .toList();
    return Scaffold(
                  ...
                  children: [
                    ...
                    Row(children: [
                      const CircleAvatar(
                          radius: 32,
                          backgroundImage:
                              AssetImage('assets/images/profile_kamal.jpg')),
                    ...
                    Wrap(spacing: 12, runSpacing: 12, children: [
                      _metric('Registered Boats', store.ownedBoats.length,
                          Icons.directions_boat),
                      _metric(
                          'Certified',
                          store.ownedBoats
                              .where((boat) =>
                                  boat.status == CertificationStatus.certified)
                              .length,
                          Icons.verified),
                      _metric('Active Trips', active.length, Icons.sailing),
                      _metric(
                          'Upcoming Trips', upcoming.length, Icons.schedule),
                      _metric(
                          'Pending Approvals',
                          store.ownedBoats
                              .where((boat) =>
                                  boat.status == CertificationStatus.pending ||
                                  boat.status ==
                                      CertificationStatus.underReview)
                              .length,
                          Icons.hourglass_top),
                      _metric('Notifications', store.unreadCount,
                          Icons.notifications)
                    ]),
                    const SizedBox(height: 28),
                    const Text('My Boats',
                    ...
                    ...store.ownedBoats.map((boat) => ListTile(
                        tileColor: Colors.white,
                        leading: const Icon(Icons.directions_boat),
                        title: Text(boat.name),
                        subtitle: Text(
                            '${boat.registrationNumber} · ${boat.status.name}'),
                        trailing: IconButton(
                    ...
                    const Text('Active & Upcoming Trips',
                        style: TextStyle(
                            fontSize: 20, fontWeight: FontWeight.bold)),
                    ...[...active, ...upcoming].map((trip) {
                      final boat = store.boat(trip.boatId)!;
                      return ListTile(
                          tileColor: Colors.white,
                          title: Text(boat.name),
                          subtitle: Text(
                              '${trip.destination} · ${trip.departure}\nSLCG ${trip.shoreApproval} · Wildlife Shore ${trip.wildlifeApproval}'),
                          isThreeLine: true,
                          trailing: IconButton(
                              icon: const Icon(Icons.info_outline),
                              onPressed: () => Navigator.pushNamed(
